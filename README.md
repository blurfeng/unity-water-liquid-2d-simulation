...
- [🚀 性能优化指南（物理）](#-性能优化指南物理)
  ...
  - [开启 Reuse Collision Callbacks（混色必需）](#开启-reuse-collision-callbacks混色必需)
  - [专用粒子层 + 收紧 Layer Collision Matrix](#专用粒子层--收紧-layer-collision-matrix)
- [📋 待办事项列表](#-待办事项列表)
...
> 混色基于物理引擎的接触点触发（`OnCollisionStay2D`），即仅在粒子实际接触时混色，避免每帧主动空间查询的开销。\
> 请确保在 `Project Settings > Physics 2D` 中开启 `Reuse Collision Callbacks`，以保证零 GC。详见[性能优化指南](#-性能优化指南物理)。\
> `Mix Colors When Stationary` 默认关闭：自身休眠（引擎认定静止）的粒子会跳过混色，使休眠刚体几乎零开销；如需静止时也混色可手动开启。
...
## 🚀 性能优化指南（物理）
大量流体粒子时，Unity 物理系统是主要的性能瓶颈。以下措施可显著提升千级粒子下的物理性能。

### Liquid2DPhysicsConfig 组件
由于项目级的 Physics 2D 设置不会随插件一起发布，插件提供了 `Liquid2DPhysicsConfig` 组件，在运行时应用推荐的物理设置。将它挂载到场景中任意常驻对象上即可：
- **多线程求解器**：开启 2D 物理多线程求解，大量动态刚体时可利用多核。⚠️ 注意：Unity 2D 物理按「island（接触连通体）」并行，单个 island 内部串行求解；当大量粒子堆积、互相接触连成一个巨大 island 时，多线程收益很有限。
- **物理频率（fixedDeltaTime）**：降低物理频率（如 50Hz→30Hz，即 0.0333）可**近乎线性减少 `Physics2D.Simulate` 总耗时**，是堆积场景下最有效的杠杆之一；代价是物理平滑度与高速穿透稳定性。
- **求解迭代次数**：流体粒子汤对求解精度不敏感，可降低速度/位置迭代次数（建议 4/2，引擎默认 8/3）以减少求解开销。注意此设置全局影响所有 2D 物理。
- **每 nameTag 存活上限**：设置每个 `nameTag` 的最大存活粒子数，超出时自动回收该 `nameTag` 下最旧的粒子，确定性地封顶物理最坏开销（0 表示不限制）。

### 开启 Reuse Collision Callbacks（混色必需）
混色已改为复用物理引擎已计算的接触点（`OnCollisionStay2D`），避免每帧主动 `Physics2D.OverlapCircle` 查询。\
请在 `Project Settings > Physics 2D` 中开启 `Reuse Collision Callbacks`，以保证混色回调零 GC（`Collision2D` 对象被复用）。

### 专用粒子层 + 收紧 Layer Collision Matrix
默认情况下流体粒子在 `Default` 层，会与场景中所有物体进行碰撞检测（broadphase），造成浪费。建议：
1. 新建一个专用层，例如 `Liquid`。
2. 将流体粒子预制体的 `Layer` 设置为该层。
3. 在 `Project Settings > Physics 2D > Layer Collision Matrix` 中，仅保留该层与「粒子层自身 / 阻挡物层 / 遮挡物层 / DeadZone 层」的碰撞，取消与其它无关层的勾选。

> ⚠️ 与 [Rendering Layer](#配置-rendering-layer) 类似，这些层与碰撞矩阵的配置不会随插件导入到你的项目中，需要你在自己的项目中手动建立。
