...
- [🧱 场景碰撞与阻挡](#-场景碰撞与阻挡)
  ...
  - [碰撞器交互模式（Push 推离 / Submerge 淹没）](#碰撞器交互模式push-推离--submerge-淹没)
...
> [!TIP]
> 每个碰撞器都有一个可选的 `nameTag` 列表：留空时作用于**全部**粒子；填写后**只阻挡命中列表中任一标签的粒子组**（可配置多个标签，命中其一即生效）。  
> 这让你可以做出「某些流体能穿过某些物体」之类的效果。  
> 注：受 `int` 位掩码所限，全局不同 `nameTag` 数量上限为 32，超出的标签会被忽略并打印一次警告。

### 碰撞器交互模式（Push 推离 / Submerge 淹没）
每个碰撞器都可以通过 `Collider Mode` 字段在两种交互模式之间切换，决定流体粒子与它相遇时的行为：
- **Push（推离，默认）**：碰撞器持续把流体粒子推出体外，粒子**无法穿透**。适合挡板、管道、流体容器、地形等需要严格阻挡流体的物体，也是原有的默认行为。
- **Submerge（淹没）**：流体粒子可以**穿过**碰撞器内部，碰撞器静止时流体会自然「覆盖」在它之上；碰撞器运动时会按速度排开周围流体、产生水花与波纹，同时**保留双向耦合**（碰撞器仍受流体阻力与浮力）。视觉上表现为流体「淹没 / 覆盖」碰撞器，适合做漂浮、沉入水中的杂物，得到更自然的入水观感。  
![](Documents/collider_mode_1.gif)

Submerge 模式下额外提供了一组专属参数，用来微调它与流体的互动（仅在该模式生效）：
- `Submerge Coupling`：位移耦合强度（0~1，默认 0.25）。碰撞器运动时把它覆盖的流体推向**自身速度的反方向**，让水绕过物体回流，完成「物体与水的位置交换」（上浮时前方水向下 / 外避让、后方水回填）。随碰撞器速度从 0 渐入，**静止时不扰动流体**。纯耗散、保证稳定，建议 0.15~0.4。
- `Submerge Splash Strength`：飞溅强度（默认 1，0 = 无飞溅）。碰撞器速度超过下方阈值后，沿速度反方向**非线性注入**额外速度产生喷溅（例如向下砸入水面时水往上溅）。仅在高速撞击的瞬间生效，不影响漂浮稳定。
- `Submerge Splash Threshold`：飞溅起始速度阈值（世界单位 / 秒，默认 4）。碰撞器速度超过此值才溅水花——应**高于**「力场扰动漂浮物」的速度、**低于**「自由下落砸水面」的速度。同时作为速度耦合渐入的参考速度。
- `Submerge Splash Range`：飞溅渐入速度区间（世界单位 / 秒，默认 6）。飞溅强度从 0 增至满所跨越的速度范围，越小越快达到最强飞溅。
- `Submerge Fluid Density Threshold`：施力流体密度阈值（相对静止密度的比例，默认 0.4，0 = 不过滤）。只对 SPH 密度高于此值的流体粒子施加位移 / 飞溅力——成片水体 / 水面密度高、正常受力；空中孤立的零散水滴密度低、不受力，避免物体在空中把靠近的水滴弹飞。自由水面约 0.5、孤立水滴约 0.1~0.2，取 0.35~0.45 可放过水面飞溅、挡住孤立水滴。

> [!TIP]
> Submerge 模式通常配合 [Liquid2DRigidbodyBridge 桥接组件](#liquid2drigidbodybridge-桥接组件) 一起使用，可以得到物体沉入 / 浮出水面、被水流冲走等更自然的双向交互效果。
