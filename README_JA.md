...
- [⛲ 流体スポナー設定ガイド](#-流体スポナー設定ガイド)
  - [スポーンの制御](#スポーンの制御)
- [🚀 パフォーマンスガイド（物理）](#-パフォーマンスガイド物理)
  - [Liquid2DPhysicsConfig コンポーネント](#liquid2dphysicsconfig-コンポーネント)
  - [Reuse Collision Callbacks を有効化（混色に必須）](#reuse-collision-callbacks-を有効化混色に必須)
  - [専用粒子レイヤー + Layer Collision Matrix の絞り込み](#専用粒子レイヤー--layer-collision-matrix-の絞り込み)
- [📋 Todoリスト](#-todo-リスト)
...
> 混色は物理エンジンの接触点（`OnCollisionStay2D`）によってトリガーされます。つまり、粒子が実際に接触したときのみ混色し、毎フレームの能動的な空間クエリのコストを回避します。\
> ゼロGCを保証するため、`Project Settings > Physics 2D` で `Reuse Collision Callbacks` を必ず有効にしてください。詳細は[パフォーマンスガイド](#-パフォーマンスガイド物理)を参照してください。\
> `Mix Colors When Stationary` はデフォルトでオフです：自身がスリープ中（エンジンが静止と判定）の粒子は混色をスキップし、スリープ剛体のコストをほぼゼロにします。静止時にも混色したい場合は手動で有効にしてください。
...
## 🚀 パフォーマンスガイド（物理）
大量の流体粒子がある場合、Unity の物理システムが主なパフォーマンスのボトルネックになります。以下の対策により、数千の粒子規模での物理パフォーマンスを大幅に向上させることができます。

### Liquid2DPhysicsConfig コンポーネント
プロジェクトレベルの Physics 2D 設定はプラグインと一緒に配布されないため、プラグインは `Liquid2DPhysicsConfig` コンポーネントを提供し、ランタイムで推奨される物理設定を適用します。シーン内の任意の常駐オブジェクトにアタッチしてください：
- **マルチスレッドソルバー**：2D物理マルチスレッドソルバーを有効にし、多数の動的剛体がある場合にマルチコアを活用します。⚠️ 注意：Unity の2D物理は「island（接触の連結集合）」単位で並列化され、単一の island 内部は直列で解かれます。多数の粒子が積み重なって接触し1つの巨大な island になると、マルチスレッドの効果は非常に限定的です。
- **物理頻度（fixedDeltaTime）**：物理頻度を下げる（例：50Hz→30Hz、すなわち 0.0333）と **`Physics2D.Simulate` の総時間がほぼ線形に減少し**、堆積シーンでは最も効果的なレバーの1つです。代償は物理の滑らかさと高速貫通の安定性です。
- **ソルバー反復回数**：流体粒子のスープはソルバー精度に鈍感なため、速度/位置の反復回数を下げる（推奨 4/2、エンジンのデフォルトは 8/3）ことでソルバーコストを削減できます。注意：この設定はすべての2D物理にグローバルに影響します。
- **nameTag ごとの生存上限**：`nameTag` ごとの最大生存粒子数を設定します。超過時はその `nameTag` の最古の粒子を自動的に回収し、物理の最悪コストを確定的に制限します（0は無制限）。

### Reuse Collision Callbacks を有効化（混色に必須）
混色は物理エンジンが既に計算した接触点（`OnCollisionStay2D`）を再利用するようになり、毎フレームの能動的な `Physics2D.OverlapCircle` クエリを回避します。\
混色コールバックをゼロGCにするため、`Project Settings > Physics 2D` で `Reuse Collision Callbacks` を有効にしてください（`Collision2D` オブジェクトが再利用されます）。

### 専用粒子レイヤー + Layer Collision Matrix の絞り込み
デフォルトでは流体粒子は `Default` レイヤーにあり、シーン内のすべてのオブジェクトと衝突検出（ブロードフェーズ）を行うため無駄が生じます。推奨：
1. 専用レイヤー（例：`Liquid`）を作成します。
2. 流体粒子プレハブの `Layer` をそのレイヤーに設定します。
3. `Project Settings > Physics 2D > Layer Collision Matrix` で、そのレイヤーと「粒子レイヤー自身 / 障害物レイヤー / オクルーダーレイヤー / DeadZone レイヤー」との衝突のみを残し、無関係なレイヤーとの衝突のチェックを外します。

> ⚠️ [Rendering Layer](#rendering-layerの設定) と同様に、これらのレイヤーと衝突マトリックスの設定はプラグインと一緒にプロジェクトにインポートされません。ご自身のプロジェクトで手動で設定する必要があります。
