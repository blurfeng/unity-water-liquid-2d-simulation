        ...
        public void Step(in Liquid2DSolveContext ctx, in SolverParams p, float dt)
        {
            ...
            for (int step = 0; step < substeps; step++)
            {
                ...
                if (lastStep)
                {
                    ...
                    // 把末子步密度与速度按类型 EMA 平滑写回 store（供渐变 Foam / Speed 渲染），并算 Impact 冲击泡沫累加器写回 store.renderFoam。读 _densities（末子步 DensityJob 写、之后未被覆盖）与最终 velocities，写 store.densities/renderSpeeds/renderFoam，与其它 job 不冲突。
