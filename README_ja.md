...
- [Renderer Feature設定ガイド](#renderer-feature設定ガイド)
...
## 💻 システム要件
- Unity 6000.2 以降のバージョン。（またはUnity2022.3のブランチバージョンを使用）
- URP2D レンダリングパイプライン。
- Renderer Graph フレームワークを使用してレンダリング。
...
## Renderer Feature設定ガイド
Renderer Featureは流体粒子をレンダリングし、最終的に流体効果を実現するために使用されます。\
...
## パーティクルスポナー設定ガイド
パーティクルスポナーは流体粒子を生成するために使用され、パイプや噴水のようなものです。\
以下では主に重要な機能やパラメータについて説明します。より詳細なパラメータは、Inspectorパネルのツールチップで直接確認できます。
...
- **物理システム**
  - 最適化：現在、大量のパーティクルを生成する際、Unityの物理システムが遅くなります。DOTS物理システムの使用など、より良いソリューションを見つける必要があります。
